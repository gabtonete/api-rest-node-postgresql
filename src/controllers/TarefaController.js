...
class TarefaController extends HttpController {
    setupRoutes(baseUrl) {
        ...
        this.express.put(`${baseUrl}/tarefa/:idTarefa`, this.editar.bind(this));
        this.express.delete(`${baseUrl}/tarefa/:idTarefa`, this.deletar.bind(this));
    }

    async listar(req, res) {
        try {
            const servico = new TarefaService(req.usuario.id);
            const tarefas = await servico.listar();
            res.json(tarefas);
    ...
    async editar(req, res) {
        try {
            const servico = new TarefaService(req.usuario.id);
            const resultado = await servico.editar(req.params.idTarefa, req.body);
            if (resultado.erros) {
                return res.status(400).json({
                    status: 400,
                    erro: resultado.erros
    ...
    async deletar(req, res) {
        try {
            const servico = new TarefaService(req.usuario.id);
            const resultado = await servico.deletar(req.params.idTarefa);
            if (resultado.erros) {
                return res.status(400).json({
                    status: 400,
                    erro: resultado.erros
